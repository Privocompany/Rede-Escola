...
export function DiscussionItem({
  ...
  return (
    <AccordionPrimitive.Item
      className={cn(
        "relative pl-4 mt-2 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-0.5 before:bg-gradient-to-b before:from-ui-divider before:via-ui-divider/60 before:to-ui-divider/40",
        className,
      )}
      {...props}
    />
...
export function DiscussionExpand({
  ...
  return (
    <AccordionPrimitive.Trigger
      className={cn(
        "flex flex-1 items-center gap-1 text-text-secondary text-xs font-medium transition-all hover:underline text-left [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
